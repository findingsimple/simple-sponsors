...
class Simple_Sponsors_Link_To {
	...
	function get_links() {
		...
		if ( !isset( $this->links[$blog_id] ) )
			$links_to = $this->get_meta( self::$post_type_name , '_sponsor-url' );
		else
	...
	function get_targets() {
		...
		if ( !isset( $this->targets[$blog_id] ) )
			$links_to_targets = $this->get_meta( self::$post_type_name ,'_sponsor-links-to-target' );
		else
	...
	function add_meta_box() {
		add_meta_box( 'sponsor-link', __( 'Sponsor Link', self::$text_domain  ), array( $this, 'do_meta_box' ), self::$post_type_name , 'normal', 'high' );
	}
